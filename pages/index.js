...
import myWallet_abi from "../artifacts/contracts/MyWallet.sol/MyWallet.json";
import avax from "./utils/avax-plain.png";
...
export default function HomePage() {
  ...
  const myWalletABI = myWallet_abi.abi;
  ...
  const getWalletContract = () => {
    ...
    const myWalletContract = new ethers.Contract(contractAddress, myWalletABI, signer);
 
    setMyWallet(myWalletContract);
  }

  const checkBalance = async() => {
    if (myWallet) {
      setBalance(parseFloat(ethers.utils.formatEther(await myWallet.checkBalance())).toFixed(2));
  ...
  const deposit = async () => {
    if (myWallet) {
      Swal.fire({
        ...
        input: "text",
        inputPlaceholder: "Enter Amount to Deposit 1",
        inputAttributes: {
          autocapitalize: "off",
        },
        inputValue: "",
        ...
        if (res.isConfirmed) {
          let tx = await myWallet.deposit(res.value, {value: ethers.utils.parseEther(res.value)});
          await tx.wait();
          Swal.fire("Sent!", "Successfully", "success");
          checkBalance();
        } else {
          Swal.fire("Cancelled!", "No deposit was made :)", "error");
        }
      });
    }
  };
  ...
  const withdraw = async () => {
    if (myWallet) {
      Swal.fire({
        ...
        showCancelButton: true,
        input: "text",
        inputPlaceholder: "Enter Amount to Withdraw 1",
        inputAttributes: {
          autocapitalize: "off",
        },
        inputValue: "",
        ...
        if (res.isConfirmed) {
          let tx = await myWallet.withdraw(res.value);
          await tx.wait();
          Swal.fire("Sent!", "Successfully", "success");
          checkBalance();
        } else {
          Swal.fire("Cancelled!", "Withdrawal Not sucessfull :)", "error");
        }
      });
    }
  };
  ...
  const sendEther = async(event) => {
    ...
    if (myWallet) {
      let tx = await myWallet.sendEther(sendTo,sendAmount);
      await tx.wait()
      Swal.fire(`Sent`, `${sendAmount} has been sent Sucessfully to ${sendTo}.`, "success" );
      setSendTo('');
      setSendAmount('');
      checkBalance();
    }
  ...
  const amountSent = async () => {
    // Add logic to interact with your contract to get the amount sent by an address
  };
  ...
  const initUser = () => {
    ...
    if (!ethWallet) {
      return <p>Please install Metamask in order to use this myWallet.</p>
    }

    // Check to see if user is connected. If not, connect to their account
    if (!account) {
      return  <div className="text-right">
       <button type="button" onClick={connectAccount} className="text-white focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-right inline-flex items-center bg-gray-800 border-gray-700 hover:bg-gray-700 me-2 mb-2">
    ...
    return (
      <div className="text-right">
        ...
        {account[0].slice(0, 4) + "..." + account[0].slice(-4)}
        </span>
        </button>      
      </div>
    )
  ...
  return (
    ...
    <div className="container mx-auto p-4 text-red">
      <h1 className="text-4xl font-bold text-gray-300 text-center">Blockchain Wallet <span>{initUser()}</span></h1>
            ...
            <section className="container items-center px-4 pb-12 mx-auto mt-20 lg:flex md:px-40">
                <div className="flex-1 space-y-4 sm:text-center lg:text-left">
                    <h1 className="text-4xl font-bold text-yellow-500">
                        Secured Blockchain wallet
                    </h1>
                    <p className="max-w-xl leading-relaxed text-gray-300 sm:mx-auto lg:ml-0">
                    Get started with our blockchain wallet today and take control of your cryptocurrency journey.
                    </p>
                    <div className="items-center justify-center space-y-3 sm:space-x-6 sm:space-y-0 sm:flex lg:justify-start">

                    <button className="block px-6 py-2 text-center text-white bg-yellow-600 rounded-md font-bold" onClick={deposit}>
                      Deposit
                    </button>

                      <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" onClick={withdraw}>
                        Withdraw
                      </button>
                    </div>
                </div>
    ...
    <div className="grid grid-cols-4 gap-4">
        ...
        <div className="mb-4 bg-slate-100 p-4 rounded-lg">
          <h2 className="text-2xl font-bold mb-2 text-center">Track Your Portfolio</h2>
          <p className="text-gray-600 text-center">Keep an eye on your investments with our intuitive portfolio tracking feature. Monitor the performance of your assets and make informed decisions for your financial future.</p>
        </div>
     
        
        <div className="mb-4 bg-slate-50 p-4 rounded-lg">
          <h2 className="text-2xl font-bold mb-2 text-center">Instant Transactions</h2>
          <p className="text-gray-600 text-center">Experience the speed of blockchain technology. Enjoy instant transactions with low fees, making your cryptocurrency transfers faster and more efficient.</p>
        </div>
      
        <div className="mb-4 bg-slate-100 p-4 rounded-lg">
          <h2 className="text-2xl font-bold mb-2 text-center">Secure Storage</h2>
          <p className="text-gray-600 text-center">Rest easy knowing that your cryptocurrency is stored securely. Our wallet employs state-of-the-art encryption and security measures to protect your assets from unauthorized access.</p>
        </div>

        <div className="mb-4 bg-slate-100 p-4 rounded-lg">
          <h2 className="text-2xl font-bold mb-2 text-center">Easy Transactions</h2>
          <p className="text-gray-600 text-center">Effortlessly deposit and withdraw funds with just a few clicks. Our user-friendly interface ensures a smooth experience for both beginners and experienced users.</p>
        </div>

    </div>

  <div className="grid grid-cols-2 gap-4">
  <form className="bg-slate-100 p-6 rounded-lg">
    <div className="relative">
        <label htmlFor="amount" className="sr-only">Amount</label>
        <input type="text" id="number" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="1" value={sendAmount}
          onChange={(e) => setSendAmount(e.target.value)} required />
    </div>
      <div className="relative max-w-sm my-4">
          <label htmlFor="receiverAddress" className="sr-only">Receiver Address</label>
          <input id="receiverAddress" type="text" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="0x12896191de42EF8388f2892Ab76b9a728189260A" value={sendTo}
          onChange={(e) => setSendTo(e.target.value)} required />
      </div>
    <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"  onClick={sendEther} >Send Ether</button>
  </form>

  <div>
  <div className="mb-4 bg-slate-100 p-4 rounded-lg align-middle text-center">
          <h2 className="text-2xl font-bold mb-2">Track Your Portfolio</h2>
          <button className="bg-purple-500 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded" onClick={checkBalance}>
          Check Balance
        </button>
